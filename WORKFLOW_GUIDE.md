...
## 🎙️ 6. Real-Time Live Microphone Streaming Architecture

The repository includes a production-ready **Real-Time Live Microphone Anti-Spoofing Pipeline** connecting the deployed Next.js web application to a FastAPI WebSocket inference backend running `wav2vec2_Nes2Net_X`.

```mermaid
sequenceDiagram
    autonumber
    participant Browser as 🌐 Client Browser (Next.js)
    participant Mic as 🎙️ Web Audio API (16kHz PCM)
    participant WS as ⚡ WebSocket Endpoint (/ws/live-detection)
    participant Server as 🧠 FastAPI Backend (live_server.py)
    participant Model as 🔬 Nes2Net Model (wav2vec2_Nes2Net_X)

    Browser->>Mic: Request User Media Permission
    Mic->>WS: Stream Binary Float32 Audio Chunks (4096 frames)
    WS->>Server: Accumulate into Rolling 4s Buffer (64,000 samples)
    Server->>Model: Forward Pass through Wav2Vec2 + Nes2Net
    Model-->>Server: Output Logits [Index 0 = SPOOF, Index 1 = REAL]
    Server-->>WS: Emit Real-Time JSON Result
    WS-->>Browser: Update UI Spectrum, Risk Score & Verdict Badge
```

### Key Technical Specs:
1. **Frontend Streaming (`src/hooks/use-live-mic-detection.ts`)**:
   - Captures microphone audio using `navigator.mediaDevices.getUserMedia`.
   - Downsamples & normalizes audio to **16,000 Hz Mono Float32 PCM** arrays via Web Audio `AudioContext` and `ScriptProcessorNode`.
   - Streams raw binary array buffers over WebSocket (`ws://localhost:8000/ws/live-detection` or `NEXT_PUBLIC_WS_URL`).
   - Calculates real-time Root Mean Square (RMS) volume for the 16-bar spectrum visualizer.

2. **Backend FastAPI Streaming Server (`live_server.py`)**:
   - Manages WebSocket sessions and maintains a rolling **4-second sliding window buffer** (64,000 audio samples).
   - Performs inference every $\ge 0.5$ seconds once the buffer accumulates audio.
   - Formats and returns structured JSON predictions:
     ```json
     {
       "spoof_prob": 97.56,
       "real_prob": 2.44,
...
3. **How to Run the Live Microphone Streaming System**:

```bash
# 1. Start the FastAPI Live Detection Server (Backend)
source .venv/bin/activate
...
# 3. Start Next.js Web Dashboard (Frontend)
npm run dev
# Open http://localhost:3000/live-calls/call-001 and click 'Start Live Mic Analysis'
```
